...
import { ActionBlock, AskBand, Caret, CodeCard, Completion, Line, PresetCard, ProfileCard, Shell, T, Welcome } from './design/Mocks';
...
function captionFor(section: Section, text: { kicker: string; headline: string; sub?: string }, wide: boolean, W: number, m: number, col: (n: number) => number) {
  const s = SECTIONS[section];
  return <Caption beat={BEAT} from={0} signal={s.signal} ground={s.ground} {...text} style={wide ? { width: col(5) - m - 40 } : { width: W - 2 * m }} />;
}
...
function Agents() {
  ...
  return (
    <Ground section="agents">
      <Pop beat={BEAT} at={0} x={c.x + c.w - 260 * k} y={c.y - 170 * k}>
        <Dots width={420 * k} height={300 * k} colour={v} pitch={20 * k} from="bottom-right" />
      </Pop>
      <Pop beat={BEAT} at={0} x={c.x + c.w - 110 * k} y={c.y + c.h - 90 * k}>
      ...
      <Pop beat={BEAT} at={0} x={c.x - 40 * k} y={c.y + c.h - 70 * k} rotate={-10}>
        <Sticker icon="cursor-click" size={96 * k} />
      </Pop>
      {captionFor('agents', { kicker: 'Agents · 05', headline: 'agents, in *plain sight*.', sub: 'every click is a block you can read, stop or take over.' }, wide, W, m, col)}
      <Pop beat={BEAT} at={0} x={m} y={wide ? H * 0.12 + 420 * k : c.y + c.h + 90 * k} rotate={-2}>
...
function Ports() {
  ...
  return (
    <Ground section="ports">
      <Pop beat={BEAT} at={0} x={c.x + c.w - 180 * k} y={c.y - 70 * k} rotate={8}>
      ...
      <Pop beat={BEAT} at={0} x={c.x - 70 * k} y={c.y + c.h - 110 * k}>
        <Solid shape="circle" size={130 * k} colour={g} />
      </Pop>
      {captionFor('ports', { kicker: 'Dev tools · 02', headline: 'every server.\nits *shell*.\nits page.', sub: 'the ports board traces each service to the shell that started it.' }, wide, W, m, col)}
    </Ground>
  );
}
...
function Language() {
  ...
  return (
    <Ground section="language">
      ...
      <Pop beat={BEAT} at={0} x={wide ? W - m - 190 * k : W - m - 150 * k} y={wide ? H - m - 560 * k : H * 0.56}>
        <Solid shape="half" size={150 * k} colour={t} />
      </Pop>
      <div style={{ position: 'absolute', right: m, bottom: wide ? m : H * 0.08, background: WHITE, border: `2px solid ${INK}`, boxShadow: `${8 * k}px ${8 * k}px 0 ${t}`, padding: `${26 * k}px ${34 * k}px`, width: wide ? 880 * k : W - 2 * m, boxSizing: 'border-box' }}>
        <Caption beat={BEAT} from={0} signal="teal" ground="white" kicker="Language · 01" headline={'language-aware,\neven *at the prompt*.'} sub="the same language servers as your editor." style={{ position: 'relative', left: 0, top: 0, width: '100%' }} />
      </div>
    </Ground>
  );
}
...
function Held() {
  ...
  return (
    <Ground section="held">
      ...
      <Pop beat={BEAT} at={0} x={wide ? W - m - 250 * k : W - m - 220 * k} y={wide ? H - m - 545 * k : H * 0.6} rotate={-6}>
      ...
      <div style={{ position: 'absolute', right: m, bottom: wide ? m : H * 0.08, background: WHITE, border: `2px solid ${INK}`, boxShadow: `${8 * k}px ${8 * k}px 0 ${g}`, padding: `${26 * k}px ${34 * k}px`, width: wide ? 860 * k : W - 2 * m, boxSizing: 'border-box' }}>
        <Caption beat={BEAT} from={0} signal="green" ground="white" kicker="Dev tools · 03" headline={'quit. update. crash.\nyour shells *keep running*.'} style={{ position: 'relative', left: 0, top: 0, width: '100%' }} />
      </div>
    </Ground>
  );
}
...
function IDE() {
  ...
  return (
    <Ground section="ide">
      ...
      <Pop beat={BEAT} at={0} x={W - 330 * k} y={H - 250 * k}>
...
function Yours() {
  ...
  return (
    <Ground section="yours">
      ...
      {captionFor('yours', { kicker: 'Customizable · 06', headline: 'yours, *all the way down*.', sub: 'rules, layouts and looks in luau. every colour a token.' }, wide, W, m, col)}
      {(['red', 'blue', 'gold', 'green', 'violet', 'teal'] as const).map((s, i) => (
        <Pop key={s} beat={BEAT} at={0} x={m + i * 58 * k} y={wide ? H - m - 60 * k : c.y - 110 * k}>
...
function Private() {
  ...
  return (
    <Ground section="private">
      <Img src={staticFile('lookdev/apple-outro-1240.png')} style={wide ? { position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', objectPosition: '62% 50%' } : { position: 'absolute', left: 0, right: 0, bottom: 0, width: '100%', height: '58%', objectFit: 'cover', objectPosition: '66% 50%' }} />
      {captionFor('private', { kicker: 'Private · 08', headline: 'no account.\nno server.\nno *telemetry*.', sub: 'a file in a folder is the whole account. sync is sealed with a key you copy.' }, wide, W, m, col)}
      <Pop beat={BEAT} at={0} x={m} y={wide ? H - 150 * k : H * 0.4} rotate={-3}>
...
function Home() {
  ...
  return (
    <Ground section="yours">
      <Pop beat={BEAT} at={0} x={c.x + c.w - 300 * k} y={c.y - 150 * k}>
      ...
      <Pop beat={BEAT} at={0} x={c.x - 50 * k} y={c.y - 50 * k} rotate={-10}>
        <Sticker icon="house" size={100 * k} />
      </Pop>
      {captionFor('yours', { kicker: 'Home · 07', headline: 'you, *on this machine*.', sub: 'your profile is a file. no server, nothing counted, nothing sent.' }, wide, W, m, col)}
    </Ground>
  );
}
...
function NumberCard() {
  ...
  return (
    <AbsoluteFill style={{ background: WHITE }}>
      <Pop beat={BEAT} at={0} x={W - 520 * k} y={-40 * k}>
      ...
      <Pop beat={BEAT} at={0} x={wide ? W * 0.62 : W - m - 220 * k} y={wide ? H * 0.52 : H * 0.62}>
        <Solid shape="circle" size={220 * k} colour={SIGNAL.red} />
      </Pop>
      <Pop beat={BEAT} at={0} x={wide ? W * 0.72 : m} y={wide ? H * 0.7 : H * 0.78} rotate={-8}>
